...
import net.maurerit.shoppingcart.exception.BadRequestException;
import net.maurerit.shoppingcart.exception.NotFoundException;
import net.maurerit.shoppingcart.exception.OperationNotAllowed;
import net.maurerit.shoppingcart.repo.BaseShoppingCartItemRepository;
import net.maurerit.shoppingcart.repo.BaseShoppingCartRepository;
...
public class ShoppingCartService extends AbstractService {
    ...
    @ApiOperation(notes = "Gets the current shopping cart for a given customer, if none exists then one is created", value = "Current shopping cart", nickname = "getShoppingCart", tags = {"Shopping Cart"})
    @ApiResponses({
    ...
    @ApiOperation(notes = "Returns a shopping cart", value = "Find by cart id", nickname = "getByCartId", tags = {"Shopping Cart"})
    @ApiResponses({
            @ApiResponse(code = 200, message = "Nice!", response = ShoppingCart.class),
            @ApiResponse(code = 400, message = "Invalid ID supplied", response = net.maurerit.shoppingcart.model.ApiResponse.class),
            @ApiResponse(code = 404, message = "Shopping Cart Not Found", response = net.maurerit.shoppingcart.model.ApiResponse.class)
    })
    ...
    @ApiOperation(notes = "Checks the shopping cart out (submits it to be fulfilled)", value = "Checkout with the carts contents", nickname = "checkout", tags = {"Shopping Cart"})
    @ApiResponses({
            @ApiResponse(code = 200, message = "Nice!", response = ShoppingCart.class),
            @ApiResponse(code = 404, message = "Shopping Cart Not Found", response = net.maurerit.shoppingcart.model.ApiResponse.class)
    })
    ...
    @ApiOperation(notes = "Adds an item to the cart", value = "Add item to cart", nickname = "addItemToCard", tags = {"Shopping Cart"})
    @ApiResponses({
            @ApiResponse(code = 200, message = "Nice!", response = ShoppingCart.class),
            @ApiResponse(code = 400, message = "Invalid ID supplied", response = net.maurerit.shoppingcart.model.ApiResponse.class),
            @ApiResponse(code = 404, message = "Shopping Cart Not Found", response = net.maurerit.shoppingcart.model.ApiResponse.class),
            @ApiResponse(code = 409, message = "Item already exists, modify quantity instead", response = net.maurerit.shoppingcart.model.ApiResponse.class)
    })
    ...
    public ResponseEntity<Boolean> addItemToCard (
        ...
        if ( cart != null ) {
            ShoppingCartItem dbItem = itemRepo.findOne(new ShoppingCartItemPK(cart.getShoppingCartId(), item.getItemId()));
            ...
            if ( dbItem == null ) {
                ...
                throw new OperationNotAllowed(net.maurerit.shoppingcart.model.ApiResponse.WARNING, "Item id: " + item.getItemId() + " already exists in cart: " + cart.getShoppingCartId());
            }
        }
    ...
    @ApiOperation(notes = "Update the quantity of an item in a cart", value = "Quantity Update", nickname = "updateItemQuantity", tags = {"Shopping Cart"})
    @ApiResponses({
            @ApiResponse(code = 200, message = "Nice!", response = ShoppingCart.class),
            @ApiResponse(code = 404, message = "Cart/Item not found", response = net.maurerit.shoppingcart.model.ApiResponse.class),
    })
    ...
    )
    {
        ...
        if ( cart != null && item != null) {
            ...
            if (dbItem == null) {
                throw new NotFoundException(net.maurerit.shoppingcart.model.ApiResponse.ERROR, "Item: " + itemId + " does not exist in cart: " + cartId + ".");
            } else {
                item.setShoppingCartId(cart.getShoppingCartId());
                itemRepo.save(item);
            }
            ...
            throw new NotFoundException(net.maurerit.shoppingcart.model.ApiResponse.ERROR, "Cart: " + cartId + " does not exist.");
        }
